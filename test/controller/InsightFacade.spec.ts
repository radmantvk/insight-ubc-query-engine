...
describe("InsightFacade", function () {
	...
	const datasetsToLoad: {[key: string]: string} = {
		...
		roomsEmptyValues: "./test/resources/archives/RoomsEmptyValues.zip",
		roomsInvalidGeoLocation: "./test/resources/archives/RoomsInvalidGeoLocation.zip",
		noValidRooms: "./test/resources/archives/NoValidRooms.zip",
		roomsMixOfHtml: "./test/resources/archives/roomsMixOfHtml.zip",
	};
	...
	describe("Add/Remove/List Dataset", function () {
		...
		it("Should pass a dataset with roomsMixOfHtml", function () {
			const id: string = "roomsMixOfHtml";
			const content: string = datasetContents.get("roomsMixOfHtml") ?? "";
			const expected: string[] = [id];
			return facade.addDataset(id, content, InsightDatasetKind.Rooms)
				.then((result: string[]) => {
					expect.fail("accepted an empty rooms dataset");
					// expect(result).to.deep.equal(expected);
					// expect(facade.listDatasets().then((list) => list[0].id === id));
					// // expect(facade.listDatasets().then((list) => list[0].numRows === 357));
					// expect(facade.listDatasets().then((list) => list[0].kind === "rooms"));
					// facade.listDatasets().then((list) => console.log(list[0].numRows));
				}).catch((err) => {
					// do nothing
				});
		});
		it("Rooms: Should fail to add a room dataset who's buildings aren't HTML format", function () {
			// check id is unique or concat "1" to the id until it is unique
			let id: string = "roomsNotHTMLFormat";
			const validContent: string = datasetContents.get("roomsNotHTMLFormat") ?? "";
			return facade.addDataset(id, validContent, InsightDatasetKind.Rooms)
				.then((result) => {
					expect.fail("failed to reject");
				})
				.catch((error) => {
					expect(error).to.be.instanceof(InsightError);
				});
		});
		it("Should reject a dataset with no rooms", function () {
		...
		it("Should add dataset with empty rooms values", function () {
			const id: string = "containingEmptyRooms";
			const content: string = datasetContents.get("roomsEmptyValues") ?? "";
			const expected: string[] = [id];
			return facade.addDataset(id, content, InsightDatasetKind.Rooms)
				.then((result: string[]) => {
					expect(result).to.deep.equal(expected);
